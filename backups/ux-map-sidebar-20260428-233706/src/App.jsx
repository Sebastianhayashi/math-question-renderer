...
function App() {
  ...
  function scrollToSection(sectionTitle) {
    const firstNode = mapNodes.find((n) => n.page.sectionTitle === sectionTitle);
            ...
            <nav className="flex flex-col gap-0 px-2">
              ...
              {view !== "quiz" &&
                directory.map((section) => {
                  ...
                  return (
                    <div key={section.title} className="mt-1 first:mt-0 px-3 py-1">
                      {/* Section header — click to jump */}
                      <button
                        onClick={() => scrollToSection(section.title)}
                        className="flex w-full items-center gap-2 py-2 rounded-lg group transition-all"
                      >
                        <span
                        ...
                        <span
                          className="flex-1 text-[12px] font-bold text-left leading-snug"
                          style={{ color: isCurrentSection ? "var(--color-primary)" : "#64748b" }}
                        >{section.title}</span>
                      ...
                      <div
                        className="ml-[22px] mt-0.5 mb-1 border-l-2 pl-4 flex flex-col gap-1"
                        ...
                        {section.pages.map((page) => {
                          ...
                          return (
                            <button
                              ...
                              className="flex items-center gap-2 relative py-1 text-left transition-all hover:translate-x-0.5"
                            >
                              {/* Dot */}
                              <div
                                className="absolute -left-[21px] w-3 h-3 rounded-full ring-2 ring-white shrink-0"
                      ...
                      {isCurrent ? (
                        ...
                        <div className="relative flex flex-col items-center gap-3">
                          ...
                          <div className="text-center mt-2">
                            <p className="text-[10px] font-bold text-muted">{page.sectionTitle}</p>
                            <p className="text-[13px] font-black" style={{ color: "var(--color-primary)" }}>{page.partTitle}</p>
                            <p className="text-[10px] font-black uppercase tracking-wider mt-0.5" style={{ color: "var(--color-primary)" }}>In Progress</p>
                          </div>
                        </div>
                      ) : (
                        ...
                        <div className="flex flex-col items-center gap-2">
                          <button onClick={() => startQuiz(page)}
                            className="node-button relative flex items-center gap-3 rounded-xl px-4 py-3 transition-all"
                            style={{ width: 160, background: "#fff", borderBottom: "5px solid #22C55E", boxShadow: "0 6px 16px rgba(0,0,0,0.08)" }}
                          >
                            {/* Green glow dot */}
                            ...
                            <div className="flex flex-col text-left">
                              <span className="text-[12px] font-bold text-slate-700 leading-tight">{page.partTitle}</span>
                              <span className="text-[10px] font-black uppercase tracking-wider mt-0.5" style={{ color: "var(--color-success)" }}>Completed</span>
                            </div>
                          </button>
                        </div>
                      )}
