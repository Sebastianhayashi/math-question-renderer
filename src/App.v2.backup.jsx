              ...
              {view !== "quiz" &&
                directory.map((section, sIdx) => (
                  <div key={section.title} className="mt-2 first:mt-0">
                    {/* Section header — click to jump map to this section */}
                    <button
                      onClick={() => scrollToSection(section.title)}
                      className="flex w-full items-center gap-2 px-2 py-1.5 rounded-lg transition-all hover:bg-background group"
                    >
                      <span className="flex h-5 min-w-[20px] items-center justify-center rounded-md text-[10px] font-black"
                        style={{ background: "rgba(46,103,248,0.1)", color: "var(--color-primary)" }}>
                        {sIdx + 1}
                      </span>
                      <p className="flex-1 text-[11px] font-bold text-text-main leading-tight text-left">
                        {section.title}
                      </p>
                      <span className="material-symbols-outlined text-[13px] text-muted opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontVariationSettings: "'FILL' 1" }}>
                        my_location
                      </span>
                    </button>
      ...
      <div className="relative flex-1 overflow-hidden min-w-0">
        {view === "map" ? (
          <div className="flex h-full flex-col">
            ...
            {/* Map Canvas */}
            <div ref={mapScrollRef} className="flex-1 overflow-auto">
              <div
                className="relative"
